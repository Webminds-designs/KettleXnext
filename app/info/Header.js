...
export default function Header() {
    ...
    return (
        <>
            <nav
                className={`p-4 fixed top-0 left-0 right-0 z-50 transition-transform duration-300 ${showHeader ? "translate-y-0" : "-translate-y-full"
                    }`}
            >
                <div className="container mx-auto flex justify-between items-center font-medium">
                    ...
                    {/* Desktop Navigation */}
                    <ul className="hidden lg:flex space-x-20 text-black uppercase text-sm mr-16">
                        {navItems.map((item) => (
                            <li
                                key={item.label}
                            ...
                            >
                                {item.label}
                                {active === item.target && (
                                    <span className="absolute left-0 bottom-[-4px] w-full h-[2px] bg-black"></span>
                                )}
                            </li>
                        ))}
                    </ul>
            ...
            <AnimatePresence>
                {mobileMenuOpen && (
                    <motion.div
                        ...
                        transition={{ duration: 0.3 }}
                        className="fixed top-16 left-0 right-0 lg:hidden z-45"
                    >
                        <ul className="flex flex-col items-center text-black uppercase text-sm py-4">
                            {navItems.map((item) => (
                                <li
                                    key={item.label}
                                    onClick={() => handleNavItemClick(item)}
